...
#             send_log.warning('send msg error {stuid}, errmsg: {errmsg}'.format(
#                 stuid=str(self.stuid) + ' ' + str(self.openid), errmsg=result.get('errmsg')))
#             SendTemplateMsg.send_lock.release()
#         else:
#             SendTemplateMsg.send_lock.acquire()
#             SendTemplateMsg.success_stus.append(self.stuid)
#             send_log.info(
#                 'send msg success {stuid}'.format(stuid=str(self.stuid) + ' ' + str(self.openid)))
#             SendTemplateMsg.send_lock.release()
#
#     def __get_url(self):
...
#         send_log.warning(acc_token['errmsg'])
...
#                 error_openid += 1
#                 send_log.warning(
#                     'send msg error {stuid}, errmsg: can not get openid'.format(stuid=i['stuid']))
#
#     threads = []
